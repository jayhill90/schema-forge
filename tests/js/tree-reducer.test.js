...
import {
	...
	denormalize,
	emptyState,
} from '../../src/shared/tree/normalize';
import {
	nodeDepth,
	graphNodes,
	nodePath,
} from '../../src/shared/tree/selectors';
...
describe( 'normalize / denormalize', () => {
	test( 'round-trips a tree unchanged', () => {
		...
		expect( out ).toEqual( {
			...
			root: {
				...sampleTree.root,
				options: { ...sampleTree.root.options, collapsed: false },
				properties: sampleTree.root.properties.map( ( p ) =>
					p.name === 'offers'
						? {
								...p,
								values: [
	...
	test( 'handles an empty tree', () => {
		expect( normalize( { version: 1, root: null } ) ).toEqual(
			emptyState()
		);
		expect( denormalize( emptyState() ) ).toEqual( {
			version: 1,
			root: null,
		} );
	} );
...
describe( 'treeReducer', () => {
	...
	test( 'sets a root type on an empty tree and forces graph placement', () => {
		const state = treeReducer(
			undefined,
			actions.setRootType( 'Event', 'n_x' )
		);
		expect( state.rootId ).toBe( 'n_x' );
		expect( state.nodes.n_x.options.placement ).toBe( 'graph' );
		const changed = treeReducer(
			state,
			actions.setRootType( 'MusicEvent' )
		);
	...
	test( 'adds, updates, moves and removes properties', () => {
		...
		state = treeReducer(
			state,
			actions.addProperty( 'n_root', 'sku', {
				id: 'p_sku',
				initialValue: {
					kind: 'text',
					value: '{{meta.sku}}',
		...
		expect( state.nodes.n_root.propertyIds ).toEqual( [
			'p_name',
			'p_offers',
			'p_brand',
			'p_sku',
	...
	test( 'nests and clears nodes on values', () => {
		...
		expect( nodePath( state, 'n_b2' ) ).toEqual( [
			'Product',
			'brand',
			'Brand',
		] );
		...
		state = treeReducer(
			state,
			actions.nestNode( 'v_b2', 'Organization', 'n_b3' )
		);
	...
	test( 'graph-placed nodes are listed for node: refs', () => {
		...
		state = treeReducer(
			state,
			actions.setNodeOption( 'n_offer', 'placement', 'graph' )
		);
		expect(
			graphNodes( state )
				.map( ( n ) => n.id )
				.sort()
		).toEqual( [ 'n_offer', 'n_root' ] );
		state = treeReducer(
			state,
			actions.setNodeOption( 'n_root', 'placement', 'inline' )
		);
	...
	test( 'ignores actions on unknown ids', () => {
		const state = load();
		expect( treeReducer( state, actions.removeProperty( 'nope' ) ) ).toBe(
			state
		);
		expect( treeReducer( state, actions.updateValue( 'nope', {} ) ) ).toBe(
			state
		);
		expect(
			treeReducer( state, actions.moveProperty( 'n_root', 0, 9 ) )
		).toBe( state );
	} );
} );
