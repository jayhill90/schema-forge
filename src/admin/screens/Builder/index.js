...
function BuilderInner( { templateId, navigate } ) {
	...
	useEffect( () => {
		...
		const load =
			templateId && templateId !== 'new'
				...
				: Promise.resolve( NEW_TEMPLATE );
		Promise.all( [ load, api.listTemplates().catch( () => [] ) ] )
			.then( ( [ template, templates ] ) => {
				if ( cancelled ) {
					return;
				}
				window.SchemaForgeTemplates = templates.filter(
					( t ) => t.id !== template.id
				);
				init( template );
				setLoading( false );
			...
			.catch( ( e ) => {
				if ( ! cancelled ) {
					setLoadError(
						errorMessage(
							e,
							__( 'Could not load the template.', 'schema-forge' )
						)
					);
					setLoading( false );
				}
	...
	const save = useCallback( async () => {
		...
		if ( ! state.tree.rootId ) {
			notify( __( 'Choose a root type before saving.', 'schema-forge' ), {
				status: 'error',
			} );
			return;
		}
			...
			notify(
				sprintf(
					/* translators: %d count */ __(
						'Fix %d error(s) in the Issues panel before saving.',
						'schema-forge'
					),
					errors.length
				),
		...
		const payload = {
			...
			tree: denormalize( state.tree ),
			settings: state.settings,
		};
		try {
			const saved = state.loadedId
				? await api.updateTemplate( state.loadedId, payload )
				: await api.createTemplate( payload );
			markSaved( saved.id );
			notify(
				saved.warnings && saved.warnings.length
					? sprintf(
							/* translators: %d count */ __(
								'Saved with %d warning(s).',
								'schema-forge'
							),
							saved.warnings.length
						)
					: __( 'Template saved.', 'schema-forge' )
			);
			if ( ! state.loadedId ) {
			...
			notify(
				errorMessage(
					e,
					__( 'Could not save the template.', 'schema-forge' )
				),
				{ status: 'error', explicitDismiss: true }
			);
		} finally {
			setSaving( false );
		}
	...
	const onDragEnd = ( { active, over } ) => {
		...
		if (
			a.kind === 'sortable-property' &&
			o &&
			o.kind === 'sortable-property'
		) {
			if ( a.nodeId === o.nodeId && a.index !== o.index ) {
				moveProperty( a.nodeId, a.index, o.index );
				setAnnouncement(
					sprintf(
						/* translators: 1: property 2: position */ __(
							'Moved “%1$s” to position %2$d.',
							'schema-forge'
						),
						a.propertyId && state.tree.properties[ a.propertyId ]
							? state.tree.properties[ a.propertyId ].name
							: '',
						o.index + 1
					)
				);
				...
				setAnnouncement(
					__(
						'Properties can only be reordered within the same node.',
						'schema-forge'
					)
				);
			}
		...
		if ( a.kind === 'type' ) {
			...
			if ( o && o.kind === 'sortable-property' ) {
				...
				if ( ! typeFitsProperty( core, property.name, a.typeName ) ) {
					setAnnouncement(
						sprintf(
							/* translators: 1: type 2: property */ __(
								'%1$s is not an expected type for “%2$s”. Added anyway; check the Issues panel.',
								'schema-forge'
							),
							a.typeName,
							property.name
				...
				const emptySlot = property.valueIds
					.map( ( id ) => state.tree.values[ id ] )
					.find(
						( v ) =>
							v &&
							( v.kind === 'node' || v.kind === 'repeat' ) &&
							! v.nodeId
					);
				if ( emptySlot ) {
			...
			if ( o && o.kind === 'node' ) {
				...
				addProperty( o.nodeId, '', {
					initialValue: { kind: 'node', typeName: a.typeName },
				} );
				setAnnouncement(
					sprintf(
						/* translators: %s type */ __(
							'Added a new property holding %s. Give it a name.',
							'schema-forge'
						),
						a.typeName
					)
		...
		() => ( {
			onDragStart: ( { active } ) => {
				...
				if ( d.kind === 'type' ) {
					return sprintf(
						/* translators: %s type */ __(
							'Picked up type %s. Drop it on a property to nest it, or on the canvas to make it the root.',
							'schema-forge'
						),
						d.typeName
					);
				...
				if ( d.kind === 'property' ) {
					return sprintf(
						/* translators: %s property */ __(
							'Picked up property %s. Drop it on a node.',
							'schema-forge'
						),
						d.name
					);
				...
				if ( d.kind === 'sortable-property' ) {
					return sprintf(
						/* translators: %d position */ __(
							'Picked up property at position %d. Use arrow keys to move, space to drop.',
							'schema-forge'
						),
						d.index + 1
					);
			...
			onDragOver: ( { active, over } ) => {
				...
				if (
					d.kind === 'sortable-property' &&
					t &&
					t.kind === 'sortable-property'
				) {
					return sprintf(
						/* translators: %d position */ __(
							'Over position %d.',
	...
	if ( loading ) {
		return (
			<div className="schema-forge-loading" role="status">
				<Spinner /> { __( 'Loading template…', 'schema-forge' ) }
			</div>
		);
	}
	if ( loadError ) {
		return (
			<div className="schema-forge-screen">
				...
				>
					{ __( 'Back to templates', 'schema-forge' ) }
				</Button>
			</div>
		);
	}
		...
		<PreviewTokensContext.Provider value={ previewTokens }>
			<DndContext
				...
				accessibility={ {
					...
					screenReaderInstructions: {
						draggable: __(
							'To pick up an item, press space or enter. While dragging, use the arrow keys to move it. Press space or enter again to drop, or escape to cancel. Every drag action also has a button alternative.',
							'schema-forge'
						),
					},
				} }
			>
				<div className="schema-forge-builder">
					<header className="schema-forge-builder__header">
						<Button
							...
							label={ __( 'Back to templates', 'schema-forge' ) }
							showTooltip
						/>
						<TextControl
						...
						/>
						<span
							className="schema-forge-builder__status"
							role="status"
						>
							{ state.dirty
								? __( 'Unsaved changes', 'schema-forge' )
								: __( 'Saved', 'schema-forge' ) }
						</span>
						<Button
							icon="undo"
							label={ __( 'Undo (Ctrl+Z)', 'schema-forge' ) }
							showTooltip
							disabled={ ! canUndo }
							onClick={ undo }
						/>
						...
						>
							{ state.loadedId
								? __( 'Save', 'schema-forge' )
								: __( 'Create template', 'schema-forge' ) }
						</Button>
					</header>
					<div className="schema-forge-builder__body">
						...
						<aside
							className="schema-forge-sidebar"
							aria-label={ __( 'Inspector', 'schema-forge' ) }
						>
							<TabPanel
								tabs={ [
									{
										name: 'inspector',
										title: __( 'Options', 'schema-forge' ),
									},
									{
										name: 'preview',
										title: __( 'Preview', 'schema-forge' ),
									},
									{
										name: 'issues',
										title: issues.length
											? sprintf(
													/* translators: 1: errors 2: warnings */ __(
														'Issues (%1$d/%2$d)',
														'schema-forge'
													),
													errorCount,
				...
				<DragOverlay dropAnimation={ null }>
					{ activeDrag && activeDrag.kind === 'type' && (
						<div className="schema-forge-drag-ghost">
							{ activeDrag.typeName }
						</div>
					) }
					{ activeDrag && activeDrag.kind === 'property' && (
						<div className="schema-forge-drag-ghost">
							{ activeDrag.name }
						</div>
					) }
			...
			{ confirmLeave && (
				<ConfirmModal
					title={ __( 'Discard unsaved changes?', 'schema-forge' ) }
					confirmLabel={ __( 'Discard changes', 'schema-forge' ) }
					onConfirm={ () => {
