...
import { useBuilder, usePreviewTokens, useSelection } from './BuilderContext';
...
function ResolvedTokens( { text } ) {
	...
	return (
		<ul
			className="schema-forge-resolved"
			aria-label={ __( 'Resolved preview values', 'schema-forge' ) }
		>
			{ found.map( ( t, i ) => {
				...
				return (
					...
					>
						<code>{ t.match }</code> →{ ' ' }
						{ display === null ? (
							<em>{ __( 'run preview', 'schema-forge' ) }</em>
						) : (
							<span>
								{ display.length > 80
									? display.slice( 0, 80 ) + '…'
...
function TextValue( { value, property, previewContext } ) {
	...
	const enumType = property
		? getRange( core, property.name ).find( ( r ) =>
			...
			<div className="schema-forge-value__row">
				<TextControl
					...
					placeholder={
						members.length
							? sprintf(
									/* translators: %s enumeration */ __(
										'%s member or {{token}}',
										'schema-forge'
									),
									enumType
								)
							: __( 'Text or {{token}}', 'schema-forge' )
					}
					list={ listId }
...
function RefValue( { value } ) {
	...
	const nodes = graphNodes( state.tree );
	const options = [
		{
			value: '',
			label: __( 'Choose a reference…', 'schema-forge' ),
			disabled: true,
		},
		...CORE_REFS.map( ( r ) => ( {
			value: r.target,
			label: config.hasYoast
				? sprintf(
						/* translators: %s: entity, e.g. "Site organization" */
						__( 'Yoast: %s', 'schema-forge' ),
						r.label
					)
				: sprintf(
						/* translators: %s: entity, e.g. "Site organization" */
						__( 'Site: %s', 'schema-forge' ),
						r.label
					),
		} ) ),
		...nodes.map( ( n ) => ( {
		...
		...templates
			.filter( ( t ) => t.id !== state.loadedId )
			.map( ( t ) => ( {
				value: `template:${ t.id }`,
				label: sprintf(
					/* translators: 1: name 2: type */ __(
						'Template: %1$s (%2$s)',
						'schema-forge'
					),
					t.name,
	...
	return (
		<div className="schema-forge-value__slot" data-value-id={ value.id }>
			<TypePicker
				label={
					value.kind === 'repeat'
						? __( 'Type for each item', 'schema-forge' )
						: __( 'Nested type', 'schema-forge' )
				}
				value=""
				suggested={ suggested }
					...
					suggested.length
						? sprintf(
								/* translators: %s types */ __(
									'Suggested: %s. Or drop a type from the palette here.',
									'schema-forge'
								),
								suggested.slice( 0, 6 ).join( ', ' )
							)
						: __(
								'Drop a type from the palette here or search.',
								'schema-forge'
							)
				}
			/>
		...
		<div className="schema-forge-value__repeat">
			<div className="schema-forge-value__row">
				<TextControl
					...
					help={ __(
						'A token that resolves to a list. Inside, use {{item.field}} or {{item.value}}.',
						'schema-forge'
					) }
					__nextHasNoMarginBottom
					__next40pxDefaultSize
...
export default function ValueEditor( {
	...
	const [ isSelected, select ] = useSelection( 'value', valueId );
	if ( ! value ) {
		return null;
	}
	return (
		...
		>
			<div className="schema-forge-value__meta">
				<span className="schema-forge-badge">
				...
				{ total > 1 && (
					<span className="schema-forge-value__order">
						<Button
							size="small"
							icon="arrow-up-alt2"
							label={ __( 'Move value up', 'schema-forge' ) }
							disabled={ index === 0 }
							onClick={ () =>
								moveValue( property.id, index, index - 1 )
							}
						/>
						<Button
							size="small"
							icon="arrow-down-alt2"
							label={ __( 'Move value down', 'schema-forge' ) }
				...
				<Button
					...
					icon="no-alt"
					isDestructive
					label={ __( 'Remove value', 'schema-forge' ) }
