...
export default function NodeCard( { nodeId, previewContext } ) {
	const { state, addProperty, clearNode, setNodeOption, select } =
		useBuilder();
	const node = state.tree.nodes[ nodeId ];
	const [ isSelected, doSelect ] = useSelection( 'node', nodeId );
	...
	const propertyDropHover =
		isOver &&
		active &&
		active.data.current &&
		active.data.current.kind === 'property';
	...
	return (
		<section
			...
			aria-label={ sprintf(
				/* translators: %s type */ __( '%s node', 'schema-forge' ),
				node.type
			) }
			data-node-id={ nodeId }
		>
			<header
				className="schema-forge-node__head"
				onClick={ doSelect }
				onKeyDown={ ( e ) => {
					if ( e.key === 'Enter' ) {
						doSelect();
			...
			>
				<Button
					...
					label={
						collapsed
							? __( 'Expand node', 'schema-forge' )
							: __( 'Collapse node', 'schema-forge' )
					}
					onClick={ ( e ) => {
						e.stopPropagation();
						setNodeOption( nodeId, 'collapsed', ! collapsed );
					} }
					aria-expanded={ ! collapsed }
				/>
				<span className="schema-forge-node__type">
					{ types.join( ' + ' ) }
				</span>
				{ isRoot && (
					<span className="schema-forge-badge schema-forge-badge--root">
						{ __( 'root', 'schema-forge' ) }
					</span>
				) }
				{ node.options.isMainEntity && (
					<span className="schema-forge-badge schema-forge-badge--main">
						{ __( 'main entity', 'schema-forge' ) }
					</span>
				) }
				{ ! isRoot && node.options.placement === 'graph' && (
					<span className="schema-forge-badge">
						{ __( 'in graph (@id)', 'schema-forge' ) }
					</span>
				) }
				<span className="schema-forge-node__count">
					{ sprintf(
				...
				<span className="schema-forge-node__actions">
					...
					{ ! isRoot && node.parentValueId && (
						<Button
							...
							icon="no-alt"
							isDestructive
							label={ __( 'Remove nested node', 'schema-forge' ) }
							onClick={ ( e ) => {
								e.stopPropagation();
								clearNode( node.parentValueId );
							} }
						/>
					) }
				</span>
				...
				<div className="schema-forge-node__body">
					...
					{ adding ? (
						...
						<div className="schema-forge-node__footer">
							...
							>
								{ __( 'Add property', 'schema-forge' ) }
							</Button>
							<span className="schema-forge-muted">
								{ __(
									'or drag a property from the palette',
									'schema-forge'
								) }
							</span>
						</div>
