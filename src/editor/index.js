...
function SchemaForgePanel() {
		...
		Promise.all( [
			...
			.catch( ( e ) =>
				setError(
					( e && e.message ) ||
						__(
							'Could not load schema assignments.',
							'schema-forge'
						)
				)
			);
	...
	const persist = async ( next ) => {
		...
		try {
			const saved = await apiFetch( {
				path: `${ NS }/posts/${ postId }/assignments`,
				method: 'PUT',
				data: next,
			} );
			setAssignments( saved );
			const resolved = await apiFetch( {
				path: `${ NS }/resolve?post_id=${ postId }`,
			} );
			setData( resolved );
			setSavedTick( ( t ) => t + 1 );
		} catch ( e ) {
			setError(
				( e && e.message ) || __( 'Could not save.', 'schema-forge' )
			);
		} finally {
			setSaving( false );
		}
	...
	let saveStatus = '';
	if ( saving ) {
		saveStatus = __( 'Saving…', 'schema-forge' );
	} else if ( savedTick > 0 ) {
		saveStatus = __( 'Saved.', 'schema-forge' );
	}

	const inherited = data.resolved.filter( ( r ) => r.source !== 'post' );
	const own = data.resolved.filter( ( r ) => r.source === 'post' );
	const disabled = data.disabled || [];
	const availableForAdd = data.available.filter(
		( t ) => ! data.resolved.find( ( r ) => r.id === t.id )
	);
	...
	return (
		<div className="schema-forge-editor-panel">
			{ inherited.length > 0 && (
				<>
					<p className="schema-forge-editor-panel__heading">
						{ __( 'Inherited templates', 'schema-forge' ) }
					</p>
					{ inherited.map( ( r ) => (
						<CheckboxControl
							key={ r.id }
							...
							onChange={ () =>
								persist( {
									...assignments,
									disable: [ ...assignments.disable, r.id ],
								} )
							}
			...
			{ disabled.length > 0 && (
				<>
					<p className="schema-forge-editor-panel__heading">
						{ __( 'Disabled on this post', 'schema-forge' ) }
					</p>
					{ disabled.map( ( r ) => (
						<CheckboxControl
							key={ r.id }
							...
							onChange={ () =>
								persist( {
									...assignments,
									disable: assignments.disable.filter(
										( id ) => id !== r.id
									),
								} )
							}
				...
				<p className="description">
					{ sprintf(
						/* translators: %s post type */ __(
							'No templates are inherited from rules for this %s.',
							'schema-forge'
						),
						postType
					) }
			...
			<ToggleControl
				...
				onChange={ ( v ) =>
					persist( { ...assignments, disableInherited: v } )
				}
				__nextHasNoMarginBottom
			/>
			<FormTokenField
				label={ __( 'Additional templates', 'schema-forge' ) }
				value={ assignments.add
				...
				<p className="description">
					{ sprintf(
						/* translators: %d count */ __(
							'%d template(s) added directly to this post.',
							'schema-forge'
						),
						own.length
					) }
			...
			{ window.SchemaForgeEditor && window.SchemaForgeEditor.adminUrl && (
				<Button
					variant="link"
					href={ window.SchemaForgeEditor.adminUrl }
					target="_blank"
					rel="noreferrer"
				>
					{ __( 'Manage schema templates ↗', 'schema-forge' ) }
				</Button>
			) }
		</div>
	);
...
registerPlugin( 'schema-forge', {
	render: () => (
		<PluginDocumentSettingPanel
			name="schema-forge"
			title={ __( 'Schema templates', 'schema-forge' ) }
			className="schema-forge-document-panel"
		>
			<SchemaForgePanel />
		</PluginDocumentSettingPanel>
	),
